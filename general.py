...
def plot_company_dataxxz(data, List_of_companies):
    ...
    for company in List_of_companies:
        ...
        company_data['Number of Moderated Content'].append(int(num_actions))
        total_sum_all_companies += num_actions

    # Add the total to the data after the loop
    company_data['Company'].append('Total')
    company_data['Number of Moderated Content'].append(total_sum_all_companies)
...
def plot_company_dataxxz_normalized(data, List_of_companies):
    ...
    # Normalize the data and format as percentage
    df_company['Normalized Actions (%)'] = (df_company['Number of Moderated Content'] / total_sum_all_companies) * 100
    df_company['Normalized Actions (%)'] = df_company['Normalized Actions (%)'].map(lambda x: f'{x:.2f}%')
    ...
    ax.set_title('Number of Actions per Company')
...
def plot_content_type_totals(data):
    ...
    '["CONTENT_TYPE_VIDEO","CONTENT_TYPE_TEXT","CONTENT_TYPE_IMAGE"]': 'VIDEO/TEXT/IMAGE',
    ...
    '["CONTENT_TYPE_IMAGE","CONTENT_TYPE_TEXT","CONTENT_TYPE_AUDIO","CONTENT_TYPE_VIDEO"]': 'IMAGE/TEXT/AUDIO/VIDEO',
    '["CONTENT_TYPE_IMAGE","CONTENT_TYPE_TEXT","CONTENT_TYPE_VIDEO"]': 'IMAGE/TEXT/VIDEO',
    '["CONTENT_TYPE_VIDEO","CONTENT_TYPE_TEXT","CONTENT_TYPE_OTHER"]': 'VIDEO/TEXT/OTHER',
    '["CONTENT_TYPE_TEXT","CONTENT_TYPE_OTHER","CONTENT_TYPE_VIDEO"]': 'TEXT/OTHER/VIDEO',
    '["CONTENT_TYPE_OTHER","CONTENT_TYPE_TEXT","CONTENT_TYPE_VIDEO"]': 'OTHER/TEXT/VIDEO',
    '["CONTENT_TYPE_OTHER","CONTENT_TYPE_VIDEO","CONTENT_TYPE_TEXT"]': 'OTHER/VIDEO/TEXT'
}
...
def plot_moderation_action_totals(data):
    ...
    '["DECISION_VISIBILITY_CONTENT_AGE_RESTRICTED","DECISION_VISIBILITY_CONTENT_DISABLED"]': 'AGE RESTRICTED/DISABLED',
    '["DECISION_VISIBILITY_CONTENT_LABELLED"]': 'LABELLED'
}
...
def plot_harm_totals_per_company(data):
    ...
    for company, harms in harm_totals_per_company.items():
        ...
        # Add total for each company
        data_for_df['Company'].append(company)
        data_for_df['Number of Moderated Content'].append(int(total_harm_totals[company]))
...
def plot_content_type_totals_per_company(data):
    ...
    for company, content_types in content_type_totals_per_company.items():
        ...
        data_for_df['Number of Moderated Content'].append(total_content_type_totals[company])
    ...
    # Move 'Total' column to the last position
...
def sum_reports_per_moderation_action_per_company(data):
    ...
    for company, moderation_actions in moderation_action_totals_per_company.items():
        ...
        data_for_df['Number of Moderated Content'].append(str(int(total_moderation_action_totals_per_company[company])))
...
def plot_automation_status_table_general(data):
    ...
    # Prepare data for DataFrame
    data_for_df = {'Automation Status': [], 'Number of Moderated Content': []}
    ...
    data_for_df['Number of Moderated Content'].append(total_sum)
...
def sum_reports_per_harm_per_moderation_action(data):
    ...
    df_harm_moderation_action['Moderation Action'] = df_harm_moderation_action['Moderation Action'].map(visibility_descriptionss)
...
def plot_content_type_automation_status_two(data):
    ...
    # Normalize the values and convert to percentages
    pivot_df_chart.iloc[:, 1:] = pivot_df_chart.iloc[:, 1:].div(pivot_df_chart.iloc[:, 1:].sum(axis=1), axis=0) * 100
    
    # Ensure values remain numeric for plotting
    pivot_df_chart.iloc[:, 1:] = pivot_df_chart.iloc[:, 1:].round(2)
    ...
    # Convert numbers to strings with '%' sign for display
    for col in pivot_df_chart.columns[1:]:
        pivot_df_chart[col] = pivot_df_chart[col].astype(str) + '%'
...
def generate_content_type_moderation_action_figure(data):
    ...
    for (content_type, moderation_action), total_actions in content_type_moderation_action_totals.items():
        data_for_df['Content Type'].append(content_type)
        data_for_df['Moderation Action'].append(moderation_action)
        data_for_df['Number of Moderated Content'].append(str(int(total_actions)))
...
def generate_moderation_action_automation_status_figure(data):
    ...
    for (moderation_action, automation_status), total_actions in moderation_action_automation_status_totals.items():
        data_for_df['Moderation Action'].append(moderation_action)
        data_for_df['Automation Status'].append(automation_status)
        data_for_df['Number of Moderated Content'].append(int(total_actions))  # Changed to int directly
